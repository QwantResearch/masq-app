...
describe('masq protocol', function () {
  ...
  // it('should delete the userApp-db after masqAccessGranted message because of a connection failure', (done) => {
  //   const channelName = 'channel111'
  //   const hub = signalhub(channelName, 'localhost:8080')
  //   const sw = swarm(hub)
  //   let dbName = null
  ...
  //     expect(await dbExists(dbName)).to.be.false
  //     setTimeout(done, 1000)
  //   })
  ...
  //         dbName = `app-${masq.profileId}-${userAppDbId}`
  //         expect(await dbExists(dbName)).to.be.true
  ...
  //   masq.handleUserAppLogin(channelName, keyBase64, `someAppId${channelName}`)
  //     .then(() => masq.handleUserAppRegister(true))
  // })
  it('should clean after masqAccessGranted message and start the register again succesfully', (done) => {
    const channelName2 = 'channel100'
    const channelName3 = 'channel101'
    ...
    sw2.once('close', async () => {
      await wait(15000)
      masq.handleUserAppLogin(channelName2, keyBase64, 'someAppId4')
        .then(() => masq.handleUserAppRegister(true))
    })
    ...
    masq.handleUserAppLogin(channelName3, keyBase64, 'someAppId3')
      .then(() => masq.handleUserAppRegister(true))
  })
})
